...
async function trackExport(key, count) {
  const c = await getClient();
  const date = todayKey();
  await c.hIncrBy(`usage:${key}:${date}`, 'exports', count || 1);
  await c.sAdd(`usage-dates:${key}`, date);
}

async function trackHeartbeat(key, seconds) {
  const c = await getClient();
  const date = todayKey();
  await c.hIncrBy(`usage:${key}:${date}`, 'timeSeconds', seconds || 0);
  await c.sAdd(`usage-dates:${key}`, date);
}

async function getUsage(key, days) {
  const c = await getClient();
  const dates = (await c.sMembers(`usage-dates:${key}`)) || [];
  const sorted = dates.sort().reverse().slice(0, days || 30);

  const results = await Promise.all(
    sorted.map(async (date) => {
      const data = await c.hGetAll(`usage:${key}:${date}`);
      return {
        date,
        exports: parseInt(data.exports || '0', 10),
        timeSeconds: parseInt(data.timeSeconds || '0', 10),
      };
    })
  );
...
async function getUsageTotals(key) {
  const c = await getClient();
  const dates = (await c.sMembers(`usage-dates:${key}`)) || [];
